...
class SharedPatchEmbedSimple(nn.Module):
    ...
    def forward(self, x, sample_channels=None):
        B, C, H, W = x.shape

        # Reshape for per-channel projection
        x = rearrange(x, 'b (c n) h w -> (b c) n h w', n=1, c=C)
        x = self.proj(x)  # (B*C, E, h, w)
        E, h, w = x.shape[1], x.shape[2], x.shape[3]

        x = rearrange(x, '(b c) e h w -> b c e h w', b=B, c=C) 

        if sample_channels is not None:
            emb = self.channel_embed[sample_channels]
        else:
            emb = self.channel_embed.unsqueeze(0)
...
class VisionTransformer(nn.Module):
    ...
    def prepare_tokens(self, x, mask=None, **kwargs):
        B, nc, w, h = x.shape
    ...
    def forward(self, x, return_all_tokens=None, mask=None, sample_channels=None):
        # mim
        if self.masked_im_modeling:
            assert mask is not None
            x = self.prepare_tokens(x, mask=mask, sample_channels=sample_channels)
        else:
            x = self.prepare_tokens(x, sample_channels=sample_channels)
...
if __name__ == '__main__':
    ...
    for b_idx in range(bs):
        ...
        img = torch.randn(4, 224, 224)
        sample_channels = torch.randperm(total_channels)[:channel_budget]

        img_sampled = img[sample_channels]

        img_list.append(img_sampled)
        chn_list.append(sample_channels)


    

    mask = torch.randint(0, 2, (bs, 14, 14), dtype=torch.bool).cuda()
    img = torch.stack(img_list).cuda()
    chn = torch.stack(chn_list).cuda()

    out = model(img, mask=mask, sample_channels=chn)
