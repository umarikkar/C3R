...
class GroupedPatchEmbedSimple(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=4, embed_dim=768, in_groups=[[0,1,2,3]], reduce_group=True, keep_channels=[3]):
        super().__init__()
...
class StreamCombiner(nn.Module):
    """ Image to Patch Embedding
    """
    def __init__(self, dim=768, in_groups=None):
        super().__init__()

        self.norm = nn.LayerNorm(dim)
            
    def forward(self, x1, x2, all_chans):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 pre_layers=None, post_layer_count=-1, keep_channels=[3], patch_embed_layer='PE', **embed_kwargs):
        super().__init__()
        ...
        self.patch_embed = GroupedPatchEmbedSimple(
                    img_size=img_size[0], patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim, in_groups=self.in_groups, keep_channels=keep_channels)
        ...
        self.combiner = StreamCombiner(dim=embed_dim, in_groups=self.in_groups)
