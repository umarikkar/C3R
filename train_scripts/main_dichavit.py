...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for it, (images, labels, masks) in enumerate(metric_logger.log_every(data_loader, 100, header)):
        ...
        with torch.amp.autocast(device_type="cuda"):
            ...
            student_output, student_global_dichavit, _ = student(images[:args.global_crops_number], mask=masks[:args.global_crops_number], first_channel_idx=first_channel_idx)
            
            # get local views
            student.module.backbone.masked_im_modeling = False

            if len(images) > args.global_crops_number:
                student_local_output, student_local_dichavit, _ = student(images[args.global_crops_number:], first_channel_idx=first_channel_idx)
