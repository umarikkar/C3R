...
def restart_from_checkpoint_3C(ckp_path, run_variables=None, **kwargs):
    """
    Re-start from checkpoint
    """
    if not os.path.isfile(ckp_path):
    ...
    checkpoint['student']['module.backbone.patch_embed.proj.weight'] = copy.deepcopy(checkpoint['student']['module.backbone.patch_embed.proj.weight'][:,1:])
    checkpoint['teacher']['backbone.patch_embed.proj.weight'] = copy.deepcopy(checkpoint['teacher']['backbone.patch_embed.proj.weight'][:,1:])

    for k in checkpoint['optimizer']['state'].keys():
        # print(k, checkpoint['optimizer']['state'][k]['exp_avg'].shape)
        if k==3:
            checkpoint['optimizer']['state'][k]['exp_avg'] = checkpoint['optimizer']['state'][k]['exp_avg'][:, 1:]
            checkpoint['optimizer']['state'][k]['exp_avg_sq'] = checkpoint['optimizer']['state'][k]['exp_avg_sq'][:, 1:]
...
class MultiCropWrapper_CVIT(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, head=None):
        super(MultiCropWrapper_CVIT, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        if head is None:
    ...
    def forward(self, x, mask=None, chans=None, return_backbone_feat=False, 
                **kwargs):
        # convert to list
        if not isinstance(x, list):
            x = [x]
        ...
        for end_idx in idx_crops:
            ...
            if chans is not None:
                inp_chan = torch.cat(chans[start_idx: end_idx])
                kwargs.update(dict(sample_channels=inp_chan))
...
class MultiCropWrapper_DiChaVIT(nn.Module):
    """
    Perform forward pass separately on each resolution input.
    The inputs corresponding to a single resolution are clubbed and single
    forward is run on the same resolution inputs. Hence we do several
    ...
    """
    def __init__(self, backbone, head=None):
        super(MultiCropWrapper_DiChaVIT, self).__init__()
        # disable layers dedicated to ImageNet labels classification
        backbone.fc, backbone.head = nn.Identity(), nn.Identity()
        self.backbone = backbone
        if head is None:
    ...
    def forward(self, x, mask=None, return_backbone_feat=False, 
        ...
        for end_idx in idx_crops:
            ...
            _out, ortho_proxy_loss, fc_idx = self.backbone(inp_x, **kwargs)

            all_dichavit_loss.append(ortho_proxy_loss)
        ...
        # mean for all crops
        all_dichavit_loss = sum(all_dichavit_loss) / len(all_dichavit_loss)
        # all_dichavit_loss = all_dichavit_loss.mean()

        if return_backbone_feat:
            return (output, output_), all_dichavit_loss, fc_idx
        return output_, all_dichavit_loss, fc_idx
